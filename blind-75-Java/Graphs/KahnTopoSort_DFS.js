...
function topoSort(V, adj) {
    ...
    function dfs(u){
        ...
        for(const v of adj[u]){
            // u has already told its list that
            //first these lists be added then u be added
            //pehle v ko insert karo
            if(!visited[v]){
                dfs(v);
            }
        }
        //last me u ko insert karo in stack
        // the moment for loop is over,
        //we are moving away from recursion so pushing 
        //u now in stack 
        stack.push(u);
    }
    for(let i=0;i<V;i++){
