...
var strStr = function (haystack, needle) {
    ...
    let n = needle.length;
    /*
    Purpose: Moves the starting index i along haystack to try all possible positions where needle might fit.

i starts at 0, moves up to m - n (so the window won't overflow the haystack).

Each value of i represents a new window where needle could match.

    */
   //For each i, try to match every character in
   //  needle with the corresponding character in haystack, starting at index i.
    for (let i = 0; i <= (m - n); i++) { // haystack loop
        for (let j = 0; j < n; j++) { // needle loop
            //If any character doesn't match:
            //haystack[i + j] !== needle[j] triggers a break,
            //  which immediately stops checking this window.
            if (haystack[i + j] !== needle[j]) {
                break
            }
            //If all characters match:
            //When j == n - 1 (last character matches), it means
            //  the whole needle was found starting at i, so the function returns i.
            if (j == n - 1) {
                return i;
            }
        }
