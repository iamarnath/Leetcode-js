...
def query_segment_tree(start,end,i,l,r,segment_tree):
   ...
   #partial overlap
   mid = (l+r) //2
   return (query_segment_tree(start,end,2*i+1,l,mid,segment_tree) +
      query_segment_tree(start,end,2*i+2,mid+1,r,segment_tree))
...
def query_sum(n,arr,q,queries):
   ...
   for i in range(0, 2*q, 2):
      ...
      result.append(query_segment_tree(start, end, 0, 0, n - 1, segment_tree))
   return result
...
arr = [1, 3, 5, 7, 9]
q = 2
queries = [1, 3, 2, 5]  # Query sum from index 1 to 3 and 2 to 5 (1-based)
print(query_sum(n, arr, q, queries))  # Output: [9, 24]